...
class ResourceMonitor:
    ...
    async def get_available_resources(self) -> Dict[str, float]:
        ...
        return {
            ...
            "memory_available": max(0, self.max_memory_usage * 100 - current_usage["memory"]),
    ...
    async def can_handle_task(self, task_requirements: Dict[str, float]) -> bool:
        ...
        memory_ok = task_requirements.get("memory_gb", 0) <= (
            available["memory_available"] / 100 * psutil.virtual_memory().total / (1024**3)
        )
        disk_ok = task_requirements.get("disk_gb", 0) <= available["disk_available"]
        
        return cpu_ok and memory_ok and disk_ok
    
    async def get_device_performance_summary(self, device_id: str) -> Optional[Dict[str, Any]]:
        ...
        recent_metrics = [m for m in metrics if m.timestamp > datetime.now() - timedelta(hours=2)]
        
        if not recent_metrics:
            return None
        
        avg_training_time = sum(m.training_time for m in recent_metrics) / len(recent_metrics)
        avg_accuracy = sum(m.model_accuracy for m in recent_metrics) / len(recent_metrics)
    ...
    async def get_optimization_suggestions(self) -> List[Dict[str, Any]]:
        ...
        if avg_cpu > 70:
            suggestions.append({
                ...
                "message": f"High CPU usage detected ({avg_cpu:.1f}%). Consider load balancing.",
        ...
        if avg_memory > 75:
            suggestions.append({
                ...
                "message": f"High memory usage detected ({avg_memory:.1f}%). Consider cache cleanup.",
        ...
        for device_id, device_metrics in self.device_metrics.items():
            if device_metrics:
                ...
                if recent_device_metrics:
                    avg_training_time = sum(m.training_time for m in recent_device_metrics) / len(recent_device_metrics)
                    
                    if avg_training_time > 300:  # 5 minutes
                        suggestions.append({
                            "type": "device_optimization",
                            "priority": "medium",
                            "message": f"Device {device_id} has slow training ({avg_training_time:.1f}s)",
                            "actions": ["reduce_model_complexity", "adjust_batch_size"]
                        })
    ...
    async def _analyze_performance(self):
        ...
        while self.is_monitoring:
            try:
                ...
                self.optimization_suggestions = await self.get_optimization_suggestions()
